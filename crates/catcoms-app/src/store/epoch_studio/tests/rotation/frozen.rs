...
fn studio_frozen_owner_store_warning_holds_across_restart_until_exact_acknowledgement() {
    ...
    for art in [false, true] {
        ...
        let mut b = budget(&mut store, &f);
        for salt in [41, 42] {
            let snapshot = StudioRecovery::snapshot(&original, None, RecoveryReason::Excluded,
                [salt; 32], &Default::default()).unwrap();
            store.update_epoch_recovery_accounted(SERVER, &f.logical, EpochRecoveryAction::Stage(snapshot),
                &clock, &mut rng(), &mut b.storage).unwrap();
        }
        let tenure = handoff(&mut f);
        drop(store);
        let mut store = open(root.path());
        warm(&f, &mut store);
        let mut b = budget(&mut store, &f);
        let (outcome, state) = store.rotate_studio_owner(SERVER, &f.group, f.target, &f.device,
            tenure, &clock, &mut rng(), &mut b).unwrap();
        ...
        let pending = store.load_epoch_recovery(SERVER, &f.logical).unwrap().eviction_pending().unwrap().unwrap();
        let journal = store.load_epoch_owner_receipts(SERVER, &f.logical).unwrap();
        ...
        warm(&f, &mut store);
        let mut b = budget(&mut store, &f);
        assert_eq!(store.rotate_studio_owner(SERVER, &f.group, f.target, &f.device,
            tenure, &clock, &mut rng(), &mut b).unwrap().0, StudioRotationOutcome::RecoveryPending);
        assert_eq!(store.load_epoch_recovery(SERVER, &f.logical).unwrap().eviction_pending().unwrap(), Some(pending));
        let RecoveryTransition::EvictionPending { oldest_snapshot, staged_snapshot, .. } = pending else { panic!() };
        let mut b = budget(&mut store, &f);
        store.update_epoch_recovery_accounted(SERVER, &f.logical,
            EpochRecoveryAction::Acknowledge { oldest_snapshot, staged_snapshot },
            &clock, &mut rng(), &mut b.storage).unwrap();
        warm(&f, &mut store);
        let mut b = budget(&mut store, &f);
        let (_, state) = store.rotate_studio_owner(SERVER, &f.group, f.target, &f.device,
            tenure, &clock, &mut rng(), &mut b).unwrap();
        assert_eq!((state.epoch(), state.phase()), (1, EpochPhase::Open));
        assert_eq!(store.load_epoch_owner_receipts(SERVER, &f.logical).unwrap().pending(), Some(&receipt));
        assert_eq!(store.load_epoch_recovery(SERVER, &f.logical).unwrap().retained().len(), 2);
        assert_eq!(f.intents(&store), 1, "takeover does not retire another author's intent");
    }
}
