# Mewtual 6e-3d; Rendezvous discovery + eclipse-resistance
...
1. **catcoms-net under `check-no-ambient.sh`**; libp2p brings its own timers; the crate may
   ...
   the injected Clock (the `RendezvousRenewal` struct in `crates/catcoms-sync/src/lib.rs`) rather
...
- **6e-3d-9 (invite rewiring + join_ns + discover→dial→join); SOUND.** No blocking defect. The
  ...
  Clock; the `RendezvousRenewal` struct in `crates/catcoms-sync/src/lib.rs`) and still accurate
  only of the CLI, where
