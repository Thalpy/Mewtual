...
| 3. Two-member collaboration and joining | **Live Save publication, bounded automatic receive/events, authenticated inventory reuse and one prepared active Studio source integrated.** Still: reconnect retry/catch-up, keyed discovery and Studio seed installation, with cold-source performance constraints retained. Reuse current native snapshot/cancellation custody. | Ordinary native Read/Create/Save exchanges durable edits/events and reopens after restart. A real two-member test accepts successive remote edits on >256-KiB art while Index/timeline refreshes preserve its prepared source. One owned graph may reuse up to 8 MiB encoded input; cold targets still refuse above 256 KiB. Inventory retains its 8 MiB read / 256 KiB cold rails. This is not full-size cold service or newcomer joining. |
...
Audited against this branch's committed history through `0d74190` (2026-09-09). This groups
...
**Next implementation target: gate 3 reconnect retry/catch-up, then keyed discovery and
Studio seed installation using the existing adapters.** Do not rebuild their inbox,
