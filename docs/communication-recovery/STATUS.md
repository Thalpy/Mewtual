...
The admission helper test exercises reply admission and restored signed records; it is not T19/T20.
...
Current agents were started in one shared checkout before the later programme plan arrived.
...
harness was run in another agent's checkout. Future independent packages should use the plan's
separate-worktree lanes, with central dispatch and evidence owned by integration.
