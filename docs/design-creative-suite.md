...
|---|---|---|
| P1 protocol core | Implemented/tested: signed operations, closes, owner receipt/fault state, epoch gate, intent/recovery models; explicit source-derived owner rotation with exact close journaling | Publication-completion driver, automatic orchestration and remaining repair/succession paths; explicit adapters are not a live scheduler |
