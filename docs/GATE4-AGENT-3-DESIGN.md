...
Revision 1 (`7efc9c2aba0a37d9aec57e268d9ff63edaca1b8a`, base
`1bcb1bca204d721b848b17c0835faf931ae930e3`) received **REQUEST CHANGES** with findings
AG3-DES-001 to AG3-DES-008 and AG3-TEST-001, plus decisions on U-1 to U-6. This revision answers
every finding, adopts every decision, and corrects the two audit claims the reviewer showed were
overstated. Scope is
...
| Repair-authorized replacement of an irrevocable owner decision | **Missing** | `epoch.rs:1993-2008`: `in_flight` is irrevocable within a tenure and `high_water` demands exact-hash equality at one epoch |
...
`plan_repair` takes `&self`, clones internally and mutates nothing. It calls the existing
`apply_repair` on the clone, so live authority, the named-pair match, the sequence rule and the
retry ordering all stay exactly where they already are and are not reimplemented.

Classification, covering every reachable fault shape. Let `S` be the selected receipt, `L` the
loser, `H` the pre-application `latest`, `E` the gate epoch, `O` the opening.

| Case | Condition | phase | receipt_hash | adopting | install |
|---|---|---|---|---|---|
| 1a | `O == Some(S)` and `H` qualifies (below) | `Closing` | `H.hash()` | `false` | none |
| 1b | `O == Some(S)` and `H` does not qualify | `Open` | none | `false` | none |
| 2 | `S.closed_epoch == E` | `Closing` | `S.hash()` | unchanged | none |
| 3 | `O == Some(L)` | `Closing` | `S.hash()` | `true` | `Some(S)` |
| 4 | `source.adopting` and none of the above | `Closing` | `S.hash()` | `true` | `Some(S)` |
| 5 | otherwise | refuse: `Held(UnsupportedShape)` | | | |
...
receipts support; they carry no ancestry chain. In every other case the head is `S`. This resolves
AG3-DES-003: in the reviewer's counterexample `O == Some(L)`, which is case 3, so the head is `S`
and the adoption planner's `latest == receipt` precondition holds.
...
adoption attempt set. Case 2 preserves the existing mode because an ordinary seal fault and an
adoption fault at this very epoch both remain valid in their own mode.
...
`high_water`, replace it with `selected` and keep the tenure selection consistent. It never sets
`in_flight`: reconciliation records what the owner already decided, it does not create a new thing
to publish. The losing receipt's bytes are retained by the caller's record (5.2), so nothing
irrevocable is erased (I-11).
...
`3 | repair | a | b | u8 applied`. Sections must appear in ascending tag order with no duplicates.
Old readers already reject any tag other than 2 (`epoch_owner.rs:141-143`), which is the stated
intent of the v2 extension.

`check_scope` gains: both receipts re-decode byte-exactly; both documents and `repair.document`
equal this document; `repair.check_evidence(&a, &b)` passes. Corruption is an error, never a silent
reset.

Bounds: `MAX_RECORD_BYTES` becomes
`MAX_OWNER_RECEIPT_JOURNAL_BYTES + MAX_CLOSE_RECORD_BYTES + 3 * MAX_RECEIPT_BYTES + 1280`, about
11.4 KiB, with `MAX_SEALED_BYTES` following. The reader still refuses anything larger before
allocating. These bytes charge the protocol allowance through the existing owner `storage_record`.
...
**S-1 issuance.** `issue_studio_repair(...)` reads the fault evidence from the actual source under
custody, refuses unless the device is the designated committer with an observed tenure, refuses
unless the supplied pair equals the held pair's hashes and the selection is one of them, derives
the sequence per 6.4, signs with `sign_in_tenure`, then calls `prepare_epoch_repair` (B1). The
signed bytes are returned only after that write returns; signing alone grants no IO authority.
...
5. `unit.apply_receipt_repair(...)`. On `Held`, save nothing and return the hold with the unchanged
   state.
6. Save the source (barrier **B2**). Fault has durably ended. `install.is_none()` returns
   `Repaired`. The owner then records B3.
...
```text
scoped query v2 = 1 | u16 doc_type | bytes channel | bytes object | bytes nonce | bytes report?
```

`report` is one canonical `Receipt`, at most `MAX_RECEIPT_BYTES`, present only when the requester
holds a receipt it believes conflicts with the provider's head. A v1 query stays byte-identical and
carries no report. The scoped query cap rises to `MAX_QUERY_V2 = MAX_QUERY + MAX_RECEIPT_BYTES`
(1280 bytes) and `queue_checkpoint_head`'s length bound to `MAX_QUERY_V2 + 144`.

Order of work on the provider, unchanged from the existing rails: the global preauth rail (20)
then request authentication, then member and epoch checks, then the per-requester rail (2) and the
pending cap (8), and only then the v2 decode. A report costs no additional rail and buys no
additional service: it rides a query that was already admitted.

Version compatibility: a provider at the old version rejects a v2 query through the existing strict
decode, so the reporter simply gets no answer and falls back to a v1 query. Older peers therefore
never receive reports, which is safe because a report only ever *adds* evidence the owner already
signed.
...
2. A **cross-tenure repair installs nothing.** The selected receipt cannot be current-owner
   verified, so `select_repaired_checkpoint` fails closed and `from_checkpoint` could not be called.
   Such a repair still ends the fault, records the evidence and screens the loser and its baseline
   descendants; convergence then happens through the current owner's ordinary first receipt, whose
   adoption stages a `Rewound` snapshot. That is the designed purpose of the v2 tenure split.
...
**Reporter.** A peer that holds a receipt `r` and, from a head answer, learns of a provider head
`h` with `conflicting_receipt_pair(document, r, h)` passing, attaches `r` as the `report` field of
its **next** query for that target. One report per query, paced by the existing four outbound head
slots and ten-second request lifetime. The reporter chooses nothing and asserts nothing about
authority.

**Provider admission**, implemented by `report_studio_fault` and reached only after the existing
authentication and rails:
...
3. The reported receipt decodes byte-exactly and `verify_signature_only()` passes.
4. The local source holds a head or opening receipt `h` with
   `conflicting_receipt_pair(document, h, reported)` passing.
5. **Admissibility.** The pair must be same-tenure, by (4), and that tenure must be the **current
   owner's** tenure, checked against the observed issuer tenure. A report naming an older tenure is
   refused in Gate 4 and the reason is stated in section 14: the purpose is to let the current owner
   repair its own equivocation, and a historical fault after succession is resolved by the current
   owner's ordinary first receipt instead. This also removes the confusion the reviewer identified
   between historical authenticity and live authority: because the pair is same-tenure and current,
   the reported receipt genuinely is a current-owner receipt and the existing typed seal is a
   legitimate admission path rather than a stretched one.
6. **No-ops.** If the local source is already Fault, or already holds a resolved repair covering the
   reported receipt (`is_repaired_loser`), the report changes nothing. A frozen pair is never
   replaced by a third receipt (I-10).
7. The fault is recorded through the **common source fences**: `resolve_studio_handoff` first, then
   the typed seal and `save_studio_source_checked`, producing a durable Fault exactly as the
   adoption path already does.
8. No winner is chosen, no checkpoint adopted, no issuer tenure inferred, no journal touched.

**Duplicates** are idempotent no-ops by (6), charged against the rails that already admitted the
query. A **failed fault write** returns no success, invalidates the budget as every other writer
does, and still answers the query; the next report retries exactly. A report **never** delays or
conditions the answer: admission runs after the answer is composed and before it is sent only if it
can be done inside the request's remaining lifetime, otherwise it is dropped and re-reported.
...
|---|---|---|---|
| B1 | owner record: repair pending, journal reconciled, stale close binding dropped | no repair exists; the fault is unchanged; the owner may decide again, possibly differently | the exact decision resumes; a different selection is refused; the owner offers the winner as an unproven hint |
...
(5.5), the Registry half of the report path, and the `registry_runtime` Fault arm attempting a
repair before giving up. Explicitly excluded: any other `DocType`, any bucket holding no Studio
...
- A cross-tenure repair ends a fault but installs nothing (6.3).
- A newcomer with unknown tenure applies no repair and converges through the owner's proof.
- A report naming an older tenure is refused in Gate 4 (6.5 rule 5); a historical fault after
  succession is resolved by the current owner's ordinary first receipt.
...
- **N5** Case 2: an ordinary seal fault at this epoch repairs into Closing under the selected
  receipt in both adoption and non-adoption modes.
...
- **N7** `resolve_repair`: replaces a losing `high_water`, clears a losing `in_flight`, returns
  `Ok(false)` when the journal is unrelated, never sets `in_flight`, and refuses an unverified or
  non-naming repair. Afterwards `prepare` accepts the next epoch's receipt, which it cannot do
  before (R13).
...
- **N17** A to B to A: B repairs A's first-tenure fault, the fault ends and nothing is installed; a
  v2 record signed in A's first tenure is refused after A returns; a v1 record is always refused.
...
- **N26** AG3-DES-001 acceptance, with no injected owner state: roll back the owner's
  ...
  Also assert: a v1 query still works; a duplicate report is a no-op; a report from a removed member
  is refused; a report naming an older tenure is refused; a third receipt never replaces a frozen
  pair; a failed fault write returns no success and the answer is still sent.
...
- **N28** AG3-DES-006: the owner's journal decision is the loser. After the repair, assert the
  **actual served receipt and proof**, a newcomer installing from that proof, restart, and that the
  owner can then issue its next ordinary receipt.
...
| M2 | recovery precedes replacement (I-2) | `stage_studio_repair_recovery` mints the capability on its `Err` path as well | N11 variant with an injected recovery write failure: "no successor bytes were written" fails |
...
| M12 | report admission (I-10) | remove the already-Fault and `is_repaired_loser` no-op checks in `report_studio_fault` | N26: a third receipt does not replace a frozen pair, and a repaired loser cannot re-fault a healed document |
...
U-1 to U-6 are answered and adopted (section 0). Two smaller questions remain:

**U-7. Should the report be admitted before or after the answer is sent?** Proposed: compose and
send the answer first, admit the report afterwards only if the request's remaining lifetime allows,
otherwise drop and let the reporter retry. This keeps a report from ever delaying a legitimate
query, at the cost of one extra round trip in the worst case.

**U-8. Should `Repairing` also cover the pre-B2 interval once a verified repair is held in memory?**
Proposed no, per U-6: before B2 the persisted state is `Fault` and the label must match what is
durable. The cost is that a slow S2 shows `fault` with no progress indication.
...
```text
Review type: design, revision 2, findings re-review.
Base: 7efc9c2aba0a37d9aec57e268d9ff63edaca1b8a (revision 1). Head: [FULL_HEAD_SHA once pushed].
Compare: https://github.com/Thalpy/Mewtual/compare/7efc9c2aba0a37d9aec57e268d9ff63edaca1b8a...[FULL_HEAD_SHA]
Original base for the whole scope: 1bcb1bca204d721b848b17c0835faf931ae930e3.
Scope/evidence: docs/GATE4-AGENT-3-DESIGN.md revision 2 and docs/GATE4-AGENT-3-STATUS.md.
Documentation only: no production code, test, shared contract document or workflow is changed, and
no Cargo command was executed. Every number is a source constant or a labelled estimate.
Dependencies unchanged: tenure seam is Agent 2's section 13.2 contract, unimplemented; source and
Prepared fence integration is Agent 1's design, unreviewed; the core signing split at e65bfd8
remains unreviewed.

This revision answers AG3-DES-001 to AG3-DES-008 and AG3-TEST-001 and adopts U-1(a), U-2, U-3,
U-4, U-5 and U-6. Section 0 is the disposition table. Two revision-1 audit claims are withdrawn or
qualified: checkpoint adoption does ingest a second receipt and durably save Fault, so the missing
piece is the reporter-to-owner direction only; and the repair signature binds the MLS group id, not
the local numeric server or the Flipnote channel.

Please check the corrections at their claimed boundaries rather than at the outcome.

AG3-DES-002: section 5.1 C-2 now computes a complete candidate on a clone, including the existing
adoption mode, and commits book, gate and mode together through C-1; five cases cover an
adoption-generated fault that already set the adoption flag, a distant adoption target with no
opening, an ordinary seal fault at this epoch, a rewind, and an explicit refusal. Try to construct
a reachable fault shape that falls into case 5, or a committed shape that the ordinary or adoption
restart validator would reject.

AG3-DES-003: the `!receipts_conflict` predicate is withdrawn as not an ancestry proof. A newer head
survives only when the source's own opening IS the selected receipt and that head seals this epoch.
N3b runs your counterexample for both winners. Check whether any retained head can still block the
adoption planner's `latest == receipt` precondition.

AG3-DES-004: disposition, continuation and completion are now read from the source through
`repair_state()`, never from the ingest outcome; every retry row flushes; the crash matrix has six
barriers with mark-applied separated; and the "Fault retained" claim after B2 is replaced by a
read-only Repairing hold that names what is actually persisted. M8 makes revision 1's behaviour the
mutant.

AG3-DES-005 and U-4: section 6.6 is one eligibility predicate used by both 5.5 and section 10,
satisfied at B2/B3 and never by a B1-only decision. N27 runs the case where the only holder of the
selected checkpoint is itself faulted. Confirm the deadlock is gone and that nothing now leaks an
unapplied decision.

AG3-DES-006: section 5.1 C-7 adds the only repair-authorized replacement of an irrevocable
decision, performed inside B1 with the losing bytes retained. Your finding is stronger than stated:
prepare_verified makes in_flight irrevocable and demands exact hash equality at one closed epoch,
so the journal was wedged, not merely mis-preferred. N7 and N28 assert the served receipt, proof,
newcomer install, restart and the next ordinary issuance.

AG3-DES-001 and U-1(a): section 6.5 is the full report contract, with a repair-independent
`conflicting_receipt_pair` checker, the v2 scoped query and its raised cap, rail ordering,
admissibility limited to a same-tenure pair in the current owner's tenure (which is also why the
existing typed seal is a legitimate admission rather than a stretched one), duplicate and failure
behaviour, version compatibility, and I-10. N26 rolls back the real owner journal and admits the
fault over the wire with no injected state. Challenge whether the admissibility rule is too narrow
or the boundary still confuses historical authenticity with live authority.

AG3-DES-007: section 9's capability is now derived by the common writer from the authenticated
durable predecessor, binds storage scope and the predecessor wrapper digest, is minted only after
the required save or flush returns, is reacquired after reopen, covers the verified empty-recovery
case explicitly, and leaves the Prepared capability, metadata link and reference checks untouched.

AG3-DES-008: section 10.3 splits capture, detached work, revalidation and commit, carries the
permit through, rebinds every coordinate, uses weak admission bookkeeping, and makes the mutation
generation mandatory over every repair write and possible-I/O path. N30 pauses a real reconstruction
and checks reference lifetime after all owners drop.

AG3-TEST-001: section 15.2 is rebuilt to your assessments. M3 targets the shared guard inside
verify_current_owner with a genuinely earlier-tenure fixture; M4 asserts at the core; M5 tests the
checker boundary directly and keeps apply_repair's comparison as labelled redundancy; M6 has an
eligible ordinary control; M7 uses visible-but-uncertain post-rename state; M8 targets continuation;
M9 targets the single effective eligibility guard; M10's assertion is corrected to "the descendant
stays stale without re-faulting" and a new M11 covers over-suppression; M12 covers report
admission. Please say whether each mutant now fails at the boundary it claims.

Two smaller questions remain open in section 16: U-7, whether a report is admitted before or after
the answer is sent, and U-8, whether Repairing should cover the pre-B2 interval.
