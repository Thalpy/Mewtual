...
| 10+ | roles hardening: **owner-signed admin grants done** (signed `owner_pubkey‖sig`, verified at read against the owner's full device id; length-prefixed group binding; reviewed) + **owner-only member removal now PROTOCOL-enforced** (`request_remove` rejects a non-owner; the committer ignores any inbound remove request not from the owner — signature-verified; THREAT-MODEL R1 closed; forged-request test) + **owner/admin file deletion done** (`delete_file` role-gated; unlists all matching entries top-down; reviewed) — remaining: replay-proof grant revocation (epoch/nonce) · **functional admin invites** = roles-based multi-committer admission (today an admin can mint but not admit; same committer-side inviter-role re-check closes THREAT-MODEL R3) · file-delete protocol gate (low stakes) · sticky/transferable ownership · blob GC after delete. See [`THREAT-MODEL.md`](THREAT-MODEL.md). | planned |
