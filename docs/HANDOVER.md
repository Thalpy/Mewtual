...
| 10+ | roles hardening: **owner-signed admin grants done** (signed `owner_pubkey‖sig`, verified at read against the owner's full device id; length-prefixed group binding; reviewed) + **owner-gated member removal/kick done** (UI Remove; MLS commit cuts the member off; ambiguous-fp-guarded; reviewed) + **owner/admin file deletion done** (`delete_file` role-gated; unlists all matching entries top-down; reviewed) — remaining: replay-proof grant revocation (epoch/nonce) · protocol-layer owner-enforcement of invite + removal + file-delete (the committer re-checking the requester's role; today all product-layer gated) · sticky/transferable ownership · blob GC after delete | planned |
