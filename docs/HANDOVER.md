...
| 9 | **disk persistence + encryption-at-rest** — designed in [`design-persistence.md`](design-persistence.md). **9a** key vault (passphrase-sealed `Dek`→`KeyHierarchy`) ✅ `2bbae5d`-prev, **9b** sealing blob store (encrypted at rest, plaintext-CID-addressed) ✅ `2bbae5d`, **9c** snapshottable MLS state (`snapshot_server`/`restore_server`; the pivotal slice — adversarially reviewed, no blocking findings) ✅. Next: 9d docs → 9e sync-state → 9f registry/reload+passphrase prompt → 9g transport → 9h per-file encryption | **in progress (9a/9b/9c done)** |
