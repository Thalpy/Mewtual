...
| 10+ | roles hardening: **owner-signed admin grants done** (signed `owner_pubkey‖sig`, verified at read against the owner's full device id; length-prefixed group binding; reviewed) — remaining: replay-proof revocation (grant epoch/nonce) · committer join-time role re-check (for future multi-committer) · member removal in the UI · sticky/transferable ownership | planned |
