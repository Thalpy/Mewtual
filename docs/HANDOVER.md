...
| 8… | **✅ rendezvous auto-discovery in the UI** — found registers at a zero-knowledge rendezvous; a joiner pasting that invite is discovered there and joins with **no hard-coded address** (`MeshHandle` for post-spawn registration, `discover_and_connect` via `DiscoveryPolicy`, fresh-invite + reload re-registration; [`design-rendezvous-ui.md`](design-rendezvous-ui.md), reviewed, no blocking findings). Remaining: chunked large-file transfer · last-copy-safe blob retention · periodic TTL re-registration + post-join steady-state discovery | rendezvous **done**; rest planned |
