...
and the original take clock survives consent pause/resume. Jukebox takes are rejected above 512 KiB before
fetch/base64 decode and use an eight-entry LRU; loads are one-running/latest, exact-call-leased and
must pass the current listing/trust gate again after fetch before cache or playback.
