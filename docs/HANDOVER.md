...
| 11n++ | **PCP/NAT-PMP + live Connectivity assistant.** Pinned `portmapper` 0.18 clients probe/map stable TCP and UDP/QUIC ports without duplicating libp2p's UPnP leases. Bounded authoritative snapshots reference-count mapping/manual owners, reject non-public CGNAT results with their reason, retry failed probes/MAP requests, remove expired mapping and relay routes from the swarm/live bootstrap/peer record, prune stale AutoNAT evidence, refresh the next displayed invite, and update onboarding + Settings live. The shared UI now includes typed one-time-help and standing-switchboard states. Already copied signed invites are immutable; current router mapping is IPv4-only; no PCPv6 pinhole client or mesh STUN path exists. | ✅ updated 2026-08-21 |
