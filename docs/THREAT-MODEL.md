...
| R6 | **Message edit / delete / react** | `edit_message`/`delete_message` gate on `author == self` (own messages; delete also allows owner/admin moderation); `toggle_reaction` keys the reaction by the caller's own fingerprint | A modified member could post a raw channel op editing/deleting **any** member's message, or forging a reaction under another member's fingerprint — the per-op inner signature signs the *delta*, not the semantic `author`/reactor (the same property that already lets a member forge a message's author on send). Low stakes — message content is not authenticated, by design. | **Low** | **Open** — accept (same posture as R2), or a later per-message author-binding hardening |
