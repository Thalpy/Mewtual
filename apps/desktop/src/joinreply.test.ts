...
test("the DM surface exposes fresh reply codes and both repair paths", () => {
  ...
  assert.match(app, /function lockScreen\([^)]*\)[\s\S]*joinReplyReady = null;[\s\S]*memberRecoveryReady = null;[\s\S]*nativeVaultLock\.begin\(finalContinuityJson\)/);
  assert.match(app, /new NativeVaultLockCoordinator\([\s\S]*invoke<NativeVaultLockOutcome>\("lock_session"/);
