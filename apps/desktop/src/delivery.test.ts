...
test("proof of arrival is never overridden by the state of the network", () => {
  ...
  assert.equal(deliveryVerdict(ev({ delivered: 1, reachable: 1, others: 2 })), "reachable");
});
...
test("the ordinary progression reads the way it should", () => {
  ...
  assert.equal(deliveryVerdict(ev({ others: 3, delivered: 2, reachable: 2 })), "reachable");
  assert.equal(deliveryVerdict(ev({ others: 3, delivered: 3, reachable: 2 })), "everyone");
});

test("every verdict has a glyph and a colour", () => {
  const all = ["pending", "waiting", "partial", "reachable", "everyone", "queued"] as const;
  for (const v of all) {
...
test("labels never say 'of 0 reachable'", () => {
  ...
  assert.equal(deliveryLabel("partial", ev({ delivered: 1, reachable: 3, others: 4 })), "delivering · 1/3 peers");
...
test("a proved count only ever rises", () => {
  ...
  // A later report that sees fewer holders has not unproved anything.
  assert.deepEqual(mergeDelivery(report(2, 2), report(1, 1)), report(2, 1));
